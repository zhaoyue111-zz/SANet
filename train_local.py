...
'''
直接训练：
    python train_local.py --dataset histopathology --epochs 10 --out-dir output

恢复训练：
默认从输出目录的 model/final.ckpt 恢复：
  python train_local.py --dataset histopathology --resume --out-dir train_output
指定某个 checkpoint 恢复：
  python train_local.py --dataset histopathology --resume --ckpt train_output/model/final.ckpt
'''
