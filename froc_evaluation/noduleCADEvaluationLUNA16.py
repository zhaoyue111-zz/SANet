...
def computeFROC_bootstrap(FROCGTList, FROCProbList, FPDivisorList, FROCImList, excludeList, numberOfBootstrapSamples=1000, confidence=0.95):
    ...
    interp_sens = np.zeros(
        (numberOfBootstrapSamples, len(all_fps)), dtype='float32')
    for i in range(numberOfBootstrapSamples):
        interp_sens[i, :] = np.interp(all_fps, fps_lists[i], sens_lists[i])

    # compute mean and CI
    sens_mean, sens_lb, sens_up = compute_mean_ci(
        interp_sens, confidence=confidence)
...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    nodOutputfile.write((60 * "*") + "\n")
    nodOutputfile.write("\n")
    with open(results_filename, newline="") as f:
    ...
    for seriesuid in tqdm(seriesUIDs):
        ...
        for noduleAnnot in noduleAnnots:
            ...
            # radiusSquared = pow((diameter * 4.0), 2.0)
            # radiusSquared = pow((diameter * 2.0), 2.0)
            ...
            if noduleAnnot.state == "Included":
                ...
                if found == True:
                    ...
                    if finalCandidate is not None:
                        ...
                        print("strange... finalcandidate is none for {0}".format(seriesuid))
                    ...
                    # if seriesuid == "449f3e1a.ee2f.4994.9d90.518626ac427c":
                        # print("FN, {}, {}, ({}, {}, {}), {}, {}".format(seriesuid, noduleAnnot.id, noduleAnnot.coordX, noduleAnnot.coordY, noduleAnnot.coordZ, float(noduleAnnot.diameter_mm), noduleAnnot.state))
        ...
        for key, candidate3 in candidates2.items():
            ...
            FPDivisorList.append(seriesuid)
            excludeList.append(False)
            FROCtoNoduleMap.append("%s,%s,%s,%s,%s,%s,%.9f" % (seriesuid, -1, candidate3.coordX,
                                                               candidate3.coordY, candidate3.coordZ, str(candidate3.id), float(candidate3.CADprobability)))
    ...
    nodOutputfile.write("    Final FROC score: %.9f\n" % (froc_score))
    nodOutputfile.close()
    ...
    return (fps, sens, thresholds, fps_bs_itp, sens_bs_mean, sens_bs_lb, sens_bs_up), listTPs, listFPs, listFNs
...
def collect(annotations_filename, annotations_excluded_filename, seriesuids_filename):
    with open(annotations_filename, newline="") as f:
        annotations = list(csv.reader(f))
    with open(annotations_excluded_filename, newline="") as f:
        annotations_excluded = list(csv.reader(f))
    with open(seriesuids_filename, newline="") as f:
        seriesUIDs_csv = list(csv.reader(f))
...
def noduleCADEvaluation(annotations_filename, annotations_excluded_filename, seriesuids_filename, results_filename, outputDir):
    ...
    print(60 * "*")
    print("outputting {0} FPs, {1} FNs, {2} TPs".format(len(listFPs), len(listFNs), len(listTPs)))
    ...
    # # analyze the subset number
    # OUTPUT_FN = False
    # OUTPUT_TP = False
    # OUTPUT_FP = False

    # # ret = re.findall("t(\d)v(\d)", results_filename)
    # # if len(ret) > 0:
    # # t_subset = int(ret[0][0])
    # # v_subset = int(ret[0][1])
    # if OUTPUT_FN:
    #     print 60 * "*"
    #     print "outputing FNs"
    #     output_nodule_details(
    #         listFNs, t_subset, 
    #     "./{}/FNs".format(outputDir))
    ...
    #     output_nodule_details(
    #         listTPs, t_subset,
    #     "./{}/TPs".format(outputDir))

    # if OUTPUT_FP:
    #     print 60 * "*"
    #     print "outputing FPs"
    #     TOP_K = 50
    #     listFPs = sorted(listFPs, key = lambda k: k.CADprobability, reverse=True)
    #     listFPs = listFPs[:TOP_K] 
    #     output_nodule_details(listFPs, t_subset, 
    #     "./details/{}/FPs".format(outputDir))

    output_nodule_details_tocsv(listTPs, "{}/TPs".format(outputDir))
    output_nodule_details_tocsv(listFPs, "{}/FPs".format(outputDir))
    output_nodule_details_tocsv(listFNs, "{}/FNs".format(outputDir))

    # else:
        # print "failed to extract subsetnames for {0}".format(results_filename)
