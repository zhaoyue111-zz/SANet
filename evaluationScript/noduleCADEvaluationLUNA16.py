...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    for seriesuid in seriesUIDs:
                        ...
                        if bOtherNodulesAsIrrelevant:  # delete marks on excluded nodules so they don't count as false positives
                            if key in candidates2.keys():
                                ...
                                ignoredCADMarksList.append(
                                    "%s,%s,%s,%s,%s,%s,%.9f" % (seriesuid, -1, candidate.coordX, candidate.coordY,
                                                                candidate.coordZ, str(candidate.id),
            ...
            if noduleAnnot.state == "Included":
                ...
                if found == True:
                    ...
                    FROCtoNoduleMap.append(
                        "%s,%s,%s,%s,%s,%.9f,%s,%.9f" % (seriesuid, noduleAnnot.id, noduleAnnot.coordX,
                                                         noduleAnnot.coordY, noduleAnnot.coordZ,
                                                         float(noduleAnnot.diameter_mm), str(finalCandidate.id),
                                                         float(finalCandidate.CADprobability)))
                    candTPs += 1
                    ...
                    FROCtoNoduleMap.append("%s,%s,%s,%s,%s,%.9f,%s,%s" % (seriesuid, noduleAnnot.id, noduleAnnot.coordX,
                                                                          noduleAnnot.coordY, noduleAnnot.coordZ,
                                                                          float(noduleAnnot.diameter_mm), int(-1),
                                                                          "NA"))
                    nodNoCandFile.write(
                        "%s,%s,%s,%s,%s,%.9f,%s\n" % (seriesuid, noduleAnnot.id, noduleAnnot.coordX, noduleAnnot.coordY,
                                                      noduleAnnot.coordZ, float(noduleAnnot.diameter_mm), str(-1)))
        ...
        for key, candidate3 in candidates2.items():
            ...
            excludeList.append(False)
            FROCtoNoduleMap.append(
                "%s,%s,%s,%s,%s,%s,%.9f" % (seriesuid, -1, candidate3.coordX, candidate3.coordY, candidate3.coordZ,
                                            str(candidate3.id), float(candidate3.CADprobability)))
...
if __name__ == '__main__':
    ...
    #
    # results_filename = '/mnt/afs2/code/nnDetection/models/Task199_AllNodule/RetinaUNetV001_D3V001_3d/fold0/test_predictions_sanet.csv'  # 3D Faster R-CNN - Res18.csv' #top5.csv'#
    annotations_filename = "/mnt/afs2/code/SANet/test_output/res/14_pretrained_rcnn20_th05_nms01/PN9/FROC/annotations.csv"
    results_filename = "/mnt/afs2/results/froc_sanet/PN9_predictions.csv"
    val_path = "/mnt/afs2/data/PN9/split/test.txt"
    noduleCADEvaluation(annotations_filename,
                        results_filename, val_path, os.path.join('/mnt/afs2/results/froc_sanet/FROC', 'rpn'))
    print("Finished!")
