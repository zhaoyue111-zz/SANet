...
def main():
    ...
    if args.focal_loss_gamma < 0:
        raise ValueError('--focal-loss-gamma must be >= 0')
    if args.focal_loss_weight < 0:
        raise ValueError('--focal-loss-weight must be >= 0')
    net_config['focal_loss_gamma'] = float(args.focal_loss_gamma)
    net_config['focal_loss_weight'] = float(args.focal_loss_weight)
    ...
    for i in tqdm(range(start_epoch, epochs + 1), desc='Total', disable=not is_main_process(args)):
        ...
        print('[focal loss: gamma %.3f, weight %.3f]' % (
